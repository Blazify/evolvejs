...
function editFile(filename) {
	readFile(`${filename}`, (err, data) => {
		...
		if(stringedData.includes("import { URL } from \"url.ts\";")) {
			stringedData = stringedData.replace("import { URL } from \"url.ts\";", "");
		}
		if(stringedData.includes("import fetch from \"node-fetch\";")) {
			stringedData = stringedData.replace("import fetch from \"node-fetch\";", "");
		}
		if(stringedData.includes("import { Data } from \"ws\"")) {
			stringedData = stringedData.replace("import { Data } from \"ws\"", "import { parse } from \"https://deno.land/std/encoding/toml\"");
		}
		...
		if(stringedData.includes("import ws, { Data } from \"ws\";")) {
			stringedData = stringedData.replace("import ws, { Data } from \"ws\";", "import { WebSocket as ws, WebSocketError } from \"https://deno.land/x/websocket@v0.0.5/mod\";");
		}
		if(stringedData.includes("this.on(\"error\", (err: Error) => {")) {
			stringedData = stringedData.replace("this.on(\"error\", (err: Error) => {", "this.on(\"error\", (err: WebSocketError) => {");
		}
		if(stringedData.includes("public send(data: Payload): void {")) {
			stringedData = stringedData.replace("public send(data: Payload): void {", "public async send(data: any): Promise<void> {");
		}
		...
		if(stringedData.includes("require(\"erlpack\").unpack(Buffer.from(data.toString(), \"binary\"))")) {
			stringedData = stringedData.replace("require(\"erlpack\").unpack(Buffer.from(data.toString(), \"binary\"))", "(await parse(new TextDecoder().decode(data as unknown as ArrayBuffer)))[0] as Payload");
		}
		if(stringedData.includes("require(\"erlpack\").pack(data)")) {
			stringedData = stringedData.replace("require(\"erlpack\").pack(data)", "payload = new TextEncoder().encode(data)");
		}
		const args = stringedData.split(/ +/g);
		for (let i = 0; i < args.length; i++) {
			if (args[i] === "from") {
				const double = args[i + 1].lastIndexOf("\"");
				args[i + 1] = args[i + 1].slice(0, double) + ".ts" + args[i + 1].slice(double);
