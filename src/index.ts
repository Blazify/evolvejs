/*
import { MessageEvents } from "./Client/Events/MessageEvents";
...
@Builder({
	"token": "NzUwMDMyNTk2OTYzOTUwNjIy.X00oSg.KlftX7S32PUu-Q_bBd7escmaBP4",
...
class Client extends EvolveClient {
    ...
    @Event()
    public newMessage(event: MessageEvents) {
    	if(!(event.message instanceof Message)) return;
		
    	if(event.message.content.toLowerCase() == "shutdown") {
    		event.channel?.send("Shutting down").then(() => {
